    ...
    switch (type) {
      case "IMAGE":
        ...
        if (isImageUrl(output)) {
          // MCP supports both embedded images and resource links
          // We'll provide both options: resource link (URL reference) + embedded image

          // 1. Add resource link for the image URL (lightweight, preserves original URL)
          console.error("[DEBUG] Adding resource link for image...");
          blocks.push({
            type: "resource_link",
            uri: output,
            name: "Generated Image",
            description: `Image generated by glif: ${output}`,
            mimeType: getMimeType(output),
          });

          // 2. Also try to embed the image as base64 (for immediate display)
          ...
          try {
            ...
            // Resource link is still available even if base64 conversion fails
          }
      ...
      case "AUDIO":
        if (isAudioUrl(output)) {
          // MCP supports both embedded audio and resource links

          // 1. Add resource link for the audio URL (lightweight, preserves original URL)
          console.error("[DEBUG] Adding resource link for audio...");
          blocks.push({
            type: "resource_link",
            uri: output,
            name: "Generated Audio",
            description: `Audio generated by glif: ${output}`,
            mimeType: getMimeType(output),
          });

          // 2. Also try to embed the audio as base64 (for immediate playback)
          console.error("[DEBUG] Converting audio URL to base64...");
          try {
            const base64Data = await urlToBase64(output);
            const mimeType = getMimeType(output);
            blocks.push({
      ...
      case "VIDEO":
        // Video content blocks are not yet supported in MCP, but we can provide a resource link
        if (isVideoUrl(output)) {
          console.error("[DEBUG] Adding resource link for video...");
          blocks.push({
            type: "resource_link",
            uri: output,
            name: "Generated Video",
            description: `Video generated by glif: ${output}`,
            mimeType: getMimeType(output),
          });
        }

        // Also add text description since MCP doesn't have native video content blocks
        blocks.push({
          type: "text",
          text: `[Video] ${output}\n\n🎥 Video URL: ${output}`,
        });
