    ...
    switch (type) {
      case "IMAGE":
        ...
        if (isImageUrl(output)) {
          console.error("[DEBUG] Adding multiple formats for image...");
          ...
          // 2. Base64 embedded image for immediate display
          try {
            console.error("[DEBUG] Converting image to base64...");
            ...
            console.error("[DEBUG] Successfully added base64 image");
          } catch (error) {
            console.error("[DEBUG] Failed to convert image to base64:", error);
          }
          
          // 3. Markdown image for maximum compatibility
          blocks.push({
            type: "text",
            text: `![Generated Image](${output})`,
          });
      ...
      case "AUDIO":
        if (isAudioUrl(output)) {
          console.error("[DEBUG] Adding multiple formats for audio...");
          ...
          // 2. Base64 embedded audio for immediate playback
          try {
            console.error("[DEBUG] Converting audio to base64...");
            const base64Data = await urlToBase64(output);
            const mimeType = getMimeType(output);
            blocks.push({
              type: "audio",
            ...
            console.error("[DEBUG] Successfully added base64 audio");
          } catch (error) {
            console.error("[DEBUG] Failed to convert audio to base64:", error);
          }
      ...
      case "VIDEO":
        if (isVideoUrl(output)) {
          console.error("[DEBUG] Adding multiple formats for video...");
          ...
          blocks.push({
            ...
            resource: {
              uri: output,
              text: `Generated video: ${output}`,
              mimeType: getMimeType(output),
            },
          });
          
          // 2. Text format for maximum compatibility
          blocks.push({
            type: "text",
            text: `🎥 Video: ${output}`,
          });
